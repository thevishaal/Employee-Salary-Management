...
def showemployeeupdate():
    ...
    def updatedata():
        ...
        xf=e6.get()
        xg=int(e7.get())
        sql="update employee set name='%s',city='%s',address='%s',email='%s',phone='%s',deptid=%d where empid=%d"%(xb,xc,xd,xe,xf,xg,xa)
        cur.execute(sql)
        db.commit()
    ...
    bt1=Button(d,text='Find',font=('biome',15),bg='white',command=finddata)
    ...
    bt2=Button(d,text='Update',font=('biome',15),bg='White',command=updatedata)
    bt2.place(x=265,y=400)

...
def showemployeesalarydelete():
    ...
    d.create_rectangle(25,25,376,275,fill='light pink')
    ...
    def filldata():
        ...
        cur=db.cursor()
        lt=[]
        sql="select empid from empsalary "
        cur.execute(sql)
        data=cur.fetchall()
        for res in data:
            lt.append(res[0])
    ...
    h1=Label(t,text='EMPLOYEE DETAIL',font=('arial',20),bg='light pink')
    h1.place(x=70,y=40)
    ...
    bt=Button(d,text='Delete',font=('biome',15),bg='light pink',command=deletedata)
    bt.place(x=140,y=200)
    
    t.mainloop()
    
#showemployeesalarydelete()